...
class CamelidaeGateAdapter(nn.Module):
    def __init__(self, config: CamelidaeConfig):
        ...
        logger.info(f"cidx -> {self.num_clusters}")
    ...
    def forward(self, input_hidden_states, output_hidden_states, router_hidden_states, cluster_idx):
        ...
        for cidx in range(self.num_clusters):
            mask = (t_cluster_idx == cidx)
            router = self.routers[f'router_{cidx}']
            router_logits[mask] = router(router_hidden_states[mask])
        ...
        input_hidden_states = input_hidden_states.view(-1, input_hidden_states.shape[-1]).repeat_interleave(self.topk,
                                                                                                            dim=0)
        expert_hidden_states = output_hidden_states.view(-1, input_hidden_states.shape[-1]).repeat_interleave(self.topk,
                                                                                                              dim=0)

        for idx in range(self.num_experts):
            for cidx in range(self.num_clusters):
                expert = self.experts[f'experts_{cidx}_{idx}']
                mask = (flat_expert_indices == idx) & (flat_cluster_indices == cidx)
                expert_hidden_states[mask] += expert(
                    input_hidden_states[mask])
