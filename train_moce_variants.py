...
from camelidae.modeling_camelidae_variant1_add_feature import LlamaForCausalLM
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    num_clusters: int = field(default=None)
    num_experts: int = field(default=None)
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        data_list = [json.loads(i.strip()) for i in files]
        # Preprocess Data
        ...
        for idx, data in enumerate(data_list):
            ...
            self.clusters.append(data["cluster"])
    ...
    def __getitem__(self, i):
        ...
        contents = {'input_id': input_ids, 'label': labels, 'cluster': cluster}

        return dict(input_ids=contents)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances): #: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        input_ids, labels, clusters = tuple(
            [instance['input_ids'][key] for instance in instances] for key in ("input_id", "label", "cluster")
        )

        # original collator -> pad
...
def train():
    ...
    model_config.topk = training_args.topk
    print('TOP N: ', model_config.topk)
    model_config.moe_scaling = 1
    model_config.num_experts = training_args.num_experts
    model_config.output_router_logits = False
    model_config.num_clusters = training_args.num_clusters
